...
func (s *SignatureintegrationsStoreSuite) TestStore() {
	...
	signatureIntegrationCount, err = store.Count(withNoAccess)
	s.NoError(err)
	...
	s.NoError(store.Upsert(ctx, signatureIntegration))
	s.ErrorIs(store.Upsert(withNoAccess, signatureIntegration), sac.ErrResourceAccessDenied)
	...
	s.ErrorIs(store.Delete(withNoAccess, signatureIntegration.GetId()), sac.ErrResourceAccessDenied)
