...
func (s *IntegrationhealthStoreSuite) TestStore() {
	...
	integrationHealthCount, err = store.Count(withNoAccess)
	s.NoError(err)
	...
	s.NoError(store.Upsert(ctx, integrationHealth))
	s.ErrorIs(store.Upsert(withNoAccess, integrationHealth), sac.ErrResourceAccessDenied)
	...
	s.ErrorIs(store.Delete(withNoAccess, integrationHealth.GetId()), sac.ErrResourceAccessDenied)
