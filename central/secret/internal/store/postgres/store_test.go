...
func (s *SecretsStoreSuite) TestStore() {
	...
	secretCount, err = store.Count(withNoAccess)
	s.NoError(err)
	s.Zero(secretCount)
	...
	s.True(secretExists)
	s.NoError(store.Upsert(ctx, secret))
	s.ErrorIs(store.Upsert(withNoAccess, secret), sac.ErrResourceAccessDenied)
	...
	s.ErrorIs(store.Delete(withNoAccess, secret.GetId()), sac.ErrResourceAccessDenied)
