...
func (s *ProcessIndicatorsStoreSuite) TestStore() {
	...
	processIndicatorCount, err = store.Count(withNoAccess)
	s.NoError(err)
	...
	s.NoError(store.Upsert(ctx, processIndicator))
	s.ErrorIs(store.Upsert(withNoAccess, processIndicator), sac.ErrResourceAccessDenied)
	...
	s.ErrorIs(store.Delete(withNoAccess, processIndicator.GetId()), sac.ErrResourceAccessDenied)
