...
func (s *RolebindingsStoreSuite) TestStore() {
	...
	k8SRoleBindingCount, err = store.Count(withNoAccess)
	s.NoError(err)
	...
	s.NoError(store.Upsert(ctx, k8SRoleBinding))
	s.ErrorIs(store.Upsert(withNoAccess, k8SRoleBinding), sac.ErrResourceAccessDenied)
	...
	s.ErrorIs(store.Delete(withNoAccess, k8SRoleBinding.GetId()), sac.ErrResourceAccessDenied)
